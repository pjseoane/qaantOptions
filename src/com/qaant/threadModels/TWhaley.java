...
public class TWhaley extends TGenericModel implements Runnable{
    ...
    public TWhaley(){super();}
    public TWhaley(Qunderlying und,char callPut, double strike,double daysToExpiration,double rate,double optionMktValue){
        super(und, callPut, strike, daysToExpiration, rate, optionMktValue);
    }
    
    public TWhaley(char tipoContrato, double underlyingValue,double underlyingHistVolatility,double dividendRate,char callPut, double strike,double daysToExpiration,double rate,double optionMktValue){
    ...
    TBlackScholes optW= new TBlackScholes(tipoContrato, underlyingValue, underlyingHistVolatility, dividendRate, callPut, strike, daysToExpiration, rate, 0);
    optW.run();
    ...
    protected double modelGetPrima(double volForLambda){
        TWhaley opt = new TWhaley(tipoContrato, underlyingValue, volForLambda,dividendRate, callPut, strike, daysToExpiration,rate,-1);
        opt.run();
         return opt.getPrima();
    }
