...
public class TBlackScholes extends TGenericModel implements Runnable{
    ...
    public TBlackScholes(){super();}
    public TBlackScholes(Qunderlying und,char callPut, double strike,double daysToExpiration,double rate,double optionMktValue){
        super(und, callPut, strike, daysToExpiration, rate, optionMktValue);
    }
    public TBlackScholes(char tipoContrato, double underlyingValue,double underlyingHistVolatility,double dividendRate,char callPut, double strike,double daysToExpiration,double rate,double optionMktValue){
    ...
    protected double modelGetPrima(double volForLambda){
        TBlackScholes opt = new TBlackScholes(tipoContrato, underlyingValue, volForLambda,dividendRate, callPut, strike, daysToExpiration,rate,-1);
        opt.run();
        return opt.getPrima();
